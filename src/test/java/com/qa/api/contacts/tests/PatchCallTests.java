...
public class PatchCallTests extends BaseTest{
	...
	Assert.assertEquals(responseGet.statusLine(),"HTTP/1.1 200 OK");
		...
		//String generateEmail=StringUtils.getRamdomEmail();
		contactData.setFirstName("Priya");
		contactData.setLastName("Mishra");
		contactData.setCity("Pune");
		
							
						
		Response responsePut=restClient.patch(BASE_URL_CONTACTS, CONTACTS_ENDPOINT+"/"+contactId, null, null, contactData, AuthType.BEARER_TOKEN, ContentType.JSON);
		Assert.assertEquals(responsePut.jsonPath().getString("firstName"), "Priya");
